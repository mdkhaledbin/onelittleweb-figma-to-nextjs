  ...
  return (
    <section id="blog">
      <div aria-label="Testimonials" className="bg-white py-12 md:py-14 mb-10">
        <Wrapper>
          <Reveal>
            <h2 className="text-center font-bold text-[#191d23] text-3xl leading-10.5 md:text-5xl">
              Check Our Clients <span className="text-brand">Review</span>
            </h2>
            <p className="mx-auto mt-4 max-w-178.25 text-center font-medium text-[#1e1e1e] text-sm leading-[1.28]">
        ...
        <div className="mt-8 flex gap-5 md:gap-10 overflow-x-auto pb-8 snap-x snap-mandatory no-scrollbar">
          {TESTIMONIALS.map((item, index) => (
            <article
              key={`${item.name}-${index}`}
              className="flex-none w-full md:w-[calc((100%-40px)/2)] lg:w-[calc((100%-80px)/3)] snap-center rounded-lg
              bg-[#F6F6F6] hover:bg-white px-11 py-10 transition-all duration-300 hover:shadow-lg hover:-translate-y-1"
            >
              <div className="flex items-center gap-6">
                {item.avatar}
                <div>
                  <h3 className="font-bold text-[#191d23] text-lg leading-[1.2]">
                    {item.name}
                  </h3>
